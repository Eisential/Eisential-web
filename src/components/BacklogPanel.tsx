...
export const BacklogPanel = ({ tasks, onTaskClick }: BacklogPanelProps) => {
  const [selectedFilter, setSelectedFilter] = useState<string>('all');
  
  // Hook para hacer este panel un drop zone
  ...
  return (
    <div 
      ref={setNodeRef} 
      className={`rounded-lg bg-white p-4 shadow border-2 transition-colors ${
        isOver ? 'bg-blue-50 border-4' : ''
      }`} 
      style={{ borderColor: isOver ? '#1E9B96' : '#1E9B70' }}
    >
